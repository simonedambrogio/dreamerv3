...
test_inner_rssm && begin
    ...
    println(ANSI_GREEN, "\n----- Testing RSSM _observe -----", ANSI_RESET)
    t = 1

    # Create separate variables for the single-step test to avoid overwriting sequence variables
    tokens_t1 = tokens[:,t,:]
    action_t1 = action[t,:]
    reset_t1 = reset[t,:]
    ...
    ps, st = Lux.setup(rng, rssm);
    # Use the new single-step variables here:
    typeof(reset_t1)
    typeof(action_t1)
    typeof(tokens_t1)
    deter_next, stoch_next = _observe(rssm, carry, tokens_t1, action_t1, reset_t1, ps, st)


    println(ANSI_BLUE, "\t Applying reset mask to state", ANSI_RESET)
    # Create the inverted mask, ready for broadcasting
    keep_mask = .!reset_t1 # Use reset_t1
    ...
    # Use action_t1 here:
    action_onehot = OneHotArrays.onehotbatch(action_t1, 0:(num_actions-1)) # Shape: (num_actions, Batch)
    action_onehot_casted = cast(action_onehot) # Cast to COMPUTE_TYPE
    # Apply reset mask to the processed action
    action_processed = action_onehot_casted .* deter_mask # Broadcast (1, Batch) mask, use a new variable name

    println(ANSI_BLUE, "\t Compute Core RSSM", ANSI_RESET)
    ps, st = Lux.setup(rng, rssm);
    # Pass the processed action_processed to _core:
    deter_next, st_updated = _core(rssm, carry.deter, carry.stoch, action_processed, ps, st)
    ...
    if unfold_RSSM_core
        ...
        text = "\t    - RMSNorm + Act"
        print(ANSI_VIOLET, text, ANSI_RESET)
        l_norm = RMSNorm((deter_dim,), 1, act; dims=(1,), init_scale=cast_ones)
...
# --- Test Gradient Computation for RSSM --- 
println(ANSI_GREEN, "\n----- Testing RSSM Gradient Computation ---", ANSI_RESET)

# 1. Instantiate RSSM and setup
# Reuse parameters from earlier tests for consistency
act_space_grad = Tools.Space(Int32, low=0, high=18); 
_token_dim_grad = size(tokens, 1) # Use token dim from encoder test
_deter_dim_grad = config["debug"]["agent"][".*\\.deter"];
_stoch_dim_grad = config["debug"]["agent"][".*\\.stoch"];
_classes_dim_grad = config["debug"]["agent"][".*\\.classes"];
_hidden_dim_grad = config["debug"]["agent"][".*\\.hidden"];
_blocks_grad = config["debug"]["agent"][".*\\.blocks"];
rssm_grad = RSSM(; act_space=act_space_grad, deter_dim=_deter_dim_grad, stoch_dim=_stoch_dim_grad, classes_dim=_classes_dim_grad, hidden_dim=_hidden_dim_grad, blocks=_blocks_grad, token_dim=_token_dim_grad);
ps_grad, st_grad = Lux.setup(rng, rssm_grad);

# 2. Prepare Inputs (reuse from earlier tests)
# Need: carry_init, tokens_in, actions_in, resets_in
carry_init_grad = initial_carry(rssm_grad, B);
# Ensure tokens, actions, resets are defined from previous sections
@assert isdefined(Main, :tokens) "tokens not defined!"
@assert isdefined(Main, :action) "action not defined!"
@assert isdefined(Main, :reset) "reset not defined!"
tokens_in_grad = tokens # Use the same tokens from encoder output
actions_in_grad = action # Use the same actions
resets_in_grad = reset   # Use the same resets

loss(rssm_grad, carry_init_grad, tokens_in_grad, actions_in_grad, resets_in_grad, ps_grad, st_grad)

# 3. Define Loss Function for Zygote
function loss_fn_rssm(m, c_in, tok_in, act_in, res_in, p, s)
    # loss function returns: carry, entry, losses, feat, metrics
    # Call the standalone loss function, passing the model `m` as the first argument
    final_c, _, losses, _, _ = loss(m, c_in, tok_in, act_in, res_in, p, s) 
    # Return a scalar loss (e.g., sum of dyn and rep mean losses)
    # Note: losses.dyn and losses.rep have shape (T, B)
    scalar_loss = mean(losses.dyn) + mean(losses.rep)
    return scalar_loss, final_c # Also need to return final carry if it were used subsequently
end

loss_fn_rssm(rssm_grad, carry_init_grad, tokens_in_grad, actions_in_grad, resets_in_grad, ps_grad, st_grad)

Zygote.withgradient(
    p_ -> begin # Differentiate w.r.t parameters p_
        # Pass model, carry, inputs, p_, state
        # State `st_grad` is treated as constant here
        loss_val, _ = loss_fn_rssm(rssm_grad, carry_init_grad, tokens_in_grad, actions_in_grad, resets_in_grad, p_, st_grad)
        loss_val
    end,
    ps_grad # Target parameters for gradient
)

# 4. Test gradient computation
println("Attempting Zygote.gradient for RSSM...")
try
    ...
    println("RSSM Gradient calculation successful!")
    println("Loss value from Zygote: ", loss_val_zygote)
    # Basic checks on gradients
    @test grad_zygote[1] isa NamedTuple
    @test !isnothing(grad_zygote[1].core.layer_deter.layer_1.weight) # Check a specific weight gradient
    @test !isnothing(grad_zygote[1].observation.logit_posterior.layer_1.bias) # Check a specific bias gradient

catch e
    println("ERROR during RSSM gradient calculation:")
    showerror(stdout, e)
    println()
