...
function check_rescaling_invariance(norm_layer, ps, st, x, scale_factor=2.0; atol=nothing)
    ...
    test_atol = isnothing(atol) ? (T == BFloat16 ? 1e-2 : 1e-5) : atol
...
@testset "Encoder Tests" begin
    ...
    @testset "2. Forward Pass" begin
        ...
        # Check state update (Encoder is stateless, state should be identical)
        @test st == st_new
    ...
    @testset "3. Check that RMSNorm layer works properly" begin
        ...
        test_shape = in_shape[filter(d -> !(d in norm_dims), 1:length(in_shape))] # Shape for scale param

        # a) Test with default scale (1.0)
        println("  Testing RMSNorm with scale = 1.0")
        # Use identity activation for testing normalization properties
        rmsn_layer = RMSNorm(test_shape, identity; dims=norm_dims, init_scale=cast_ones)
        ...
        custom_init_scale(rng, T, dims...) = fill!(similar(rand(rng, T, dims...)), T(0.5))
        # Use identity activation for testing normalization properties
        rmsn_layer_custom = RMSNorm(test_shape, identity; dims=norm_dims, init_scale=custom_init_scale)
