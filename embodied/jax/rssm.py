...
class Encoder(nj.Module):
  ...
  def __call__(self, carry, obs, reset, training, single=False):
    ...
    # print("obs: ", obs) obs{'image' : uint8(1,10,96,96,1)}
    print("step2 start")
    if self.imgkeys:
      K = self.kernel
      print("K: ", K)
      ...
      # concatenate images along the last dimension 
      x = nn.cast(jnp.concatenate(imgs, -1), force=True) / 255 - 0.5
      # assess type of x
      print("type of x: ", x.dtype)
      
      print(f"After cast and normalize: {x.shape}  (batch_size, sequence_length, height, width, channels)")
      x = x.reshape((-1, *x.shape[bdims:]))
    ...
    print("outs after concat: ", x.shape)
    
    tokens = x.reshape((*bshape, *x.shape[1:]))
    print("tokens: ", tokens.shape)
    print("tokens types : ", tokens.dtype)
...
class Decoder(nj.Module):
  ...
  def __call__(self, carry, feat, reset, training, single=False):
    ...
    K = self.kernel
    recons = {}
    bshape = reset.shape # batch_size x sequence_length
    
    inp = [nn.cast(feat[k]) for k in ('stoch', 'deter')]
    print("stoch before reshape: ", inp[0].shape)
    print("deter before reshape: ", inp[1].shape)
    inp = [x.reshape((math.prod(bshape), -1)) for x in inp]
    print("stoch after reshape: ", inp[0].shape)
    print("deter after reshape: ", inp[1].shape)
    inp = jnp.concatenate(inp, -1)
    print("deter after concat: ", inp.shape)
    ...
    if self.imgkeys:
      ...
      print("imgdep: ", self.imgdep)
      print("imgres: ", self.imgres)
      ...
      if self.bspace:
        ...
        print("x0 before blocklinear: ", x0.shape)
        x0 = self.sub('sp0', nn.BlockLinear, u, g, **self.kw)(x0)
        
        print("x0 after blocklinear: ", x0.shape)
        x0 = einops.rearrange(
            x0, '... (g h w c) -> ... h w (g c)',
            h=minres[0], w=minres[1], g=g)
        print("x0 after rearrange: ", x0.shape)
        
        print("x1 before linear: ", x1.shape)
        x1 = self.sub('sp1', nn.Linear, 2 * self.units, **self.kw)(x1)
        print("x1 after linear 1: ", x1.shape)
        x1 = nn.act(self.act)(self.sub('sp1norm', nn.Norm, self.norm)(x1))
        print("x1 after norm 1: ", x1.shape)
        print("shape: ", shape)
        x1 = self.sub('sp2', nn.Linear, shape, **self.kw)(x1)
        print("x1 after linear 2: ", x1.shape)
