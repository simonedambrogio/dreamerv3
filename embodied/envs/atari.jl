...
function step!(env::Atari, action::Dict)
    if action["reset"] || env.done
        ...
        env.duration = 0
        env.done = false
        return _obs(env, 0.0, is_first=true)
    end
    ...
    act = env.actionset[action["action"]]
...
function _render!(env::Atari)
    ...
    screen_data = reshape(getScreenRGB(env.ale), w, h, 3)  # WHC format directly
