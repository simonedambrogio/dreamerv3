...
function WorldModelAgent(config::Dict, spaces::Dict)
    ...
    encoder = Encoder(; 
        obs=spaces[:image], 
        act=eval(Meta.parse(config["enc"]["act"])), 
        mults=config["enc"]["mults"], 
        depth=config["enc"]["depth"], 
        kernel=config["enc"]["kernel"]
    ...
    rssm = StatefulRSSM(;
        ...
        free_nats=config["dyn"]["free_nats"] |> cast,
        unimix=config["dyn"]["unimix"] |> cast
    )
