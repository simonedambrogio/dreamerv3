...
function Decoder(;
    ...
    # return the encoder ------------------------------------------------------
    return Decoder(act, mults, depth, kernel, nn, obs, Tuple(depths), shape, bspace)
end

rng = Random.default_rng();
dec = Decoder(; obs=Tools.Space(UInt8, (96, 96, 1)), depth=config["debug"]["agent"][".*\\.depth"]);

batch_size = config["debug"]["batch_size"];
seq_length = config["debug"]["batch_length"];
